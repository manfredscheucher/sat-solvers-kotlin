/*********************************************************************[microsat_trace.c]***
  ...
  INSTRUMENTED copy of Marijn Heule's MicroSAT (MIT, (c) 2014-2018 Marijn Heule).
  See LICENSE. The original is kept verbatim as microsat_orig.c; this file adds a
  step-by-step decision TRACE so the Kotlin port (microsat-kotlin/) can be shadowed
  byte-for-byte against it.

  The only changes vs the original are `trace(...)` calls emitting one line per
  internal event (DECIDE / ASSIGN / CONFLICT / ANALYZE / RESTART / REDUCE / RESULT),
  gated by the LSTRACE env var so normal runs stay clean. No algorithm changes.

  Trace format (stable, both implementations must match):
    DECIDE <lit>
    ASSIGN <lit> reason=<D|F|C>     (D=decision, F=forced/unit, C=learned)
    CONFLICT
    RESTART
    REDUCE
    RESULT <SAT|UNSAT>
  Literals are the signed DIMACS literal actually assigned true.
...
static void trace_init (void) { TRACE_ON = getenv ("LSTRACE") != NULL; }
...
struct solver { // The variables in the struct are described in the initCDCL procedure
  int  *DB, nVars, nClauses, mem_used, mem_fixed, mem_max, maxLemmas, nLemmas, *buffer, nConflicts, *model,
       *reason, *falseStack, *false, *first, *forced, *processed, *assigned, *next, *prev, head, res, fast, slow, restartFactor; };
...
int solve (struct solver* S) {                                      // Determine satisfiability
  ...
  for (;;) {                                                        // Main solve loop
    ...
    if (propagate (S) == UNSAT) { TR ("RESULT UNSAT\n"); return UNSAT; } // Propagation returns UNSAT for a root level conflict

    if (S->nLemmas > old_nLemmas) {                                 // If the last decision caused a conflict
      decision = S->head;                                           // Reset the decision heuristic to head
      if (S->fast > (S->slow / 100) * S->restartFactor) {           // If fast average is substantially larger than slow average
        S->res = 0; S->fast = (S->slow / 100) * S->restartFactor; restart (S); // Restart and update the averages
...
void initCDCL (struct solver* S, int n, int m) {
  ...
  { char* lm = getenv ("LSMAXLEMMAS");    // TEST-ONLY knob (mirrors MicroSat.kt maxLemmasInit):
    if (lm) S->maxLemmas = atoi (lm); }   // override initial maxLemmas to force REDUCE on small instances
  S->restartFactor = 125;                 // default restart aggressiveness (fast > slow/100 * factor)
  { char* rf = getenv ("LSRESTARTFACTOR"); // TEST-ONLY knob (mirrors MicroSat.kt restartFactorInit):
    if (rf) S->restartFactor = atoi (rf); } // lower it to make restarts (and thus REDUCE) fire on small instances
...
int main (int argc, char** argv) {			               // The main procedure for a STANDALONE solver
  ...
  else if (parse (&S, argv[1]) == UNSAT) { TR ("RESULT UNSAT\n"); printf("s UNSATISFIABLE\n"); }  // Parse the DIMACS file in argv[1]
