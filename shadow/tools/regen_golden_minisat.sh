#!/usr/bin/env bash
# Regenerate the golden C MiniSat traces for the JVM shadow test.
#
# Pipeline (durable, kept in repo):
#   1. generate the CNF suite                   (gen_cnf.py -> shadow/cnf/*.cnf)
#   2. build the instrumented C MiniSat         (clang++ -O2 minisat_trace.cc)
#   3. for each CNF: run it with LSTRACE=1, strip the "s "/"c " status lines, and
#      save the pure event trace to shadow/golden-minisat/<name>.trace
#   4. record which CNFs contain RESTART / REDUCE lines into a manifest.
#
# The JVM shadow test (minisat/.../ShadowTraceFilesTest.kt) then reads shadow/cnf/*.cnf
# + shadow/golden-minisat/*.trace and compares the Kotlin trace at the strongest level
# that holds (L1 full trace, else L2 event/decision sequence, always L3 verdict+model).
#
# Run:  bash shadow/tools/regen_golden_minisat.sh
set -euo pipefail
here="$(cd "$(dirname "$0")" && pwd)"
shadow="$(cd "$here/.." && pwd)"
cnfdir="$shadow/cnf"
golddir="$shadow/golden-minisat"
...
echo "[2/4] building instrumented C MiniSat (clang++ -O2)"
CXX="${CXX:-clang++}"
...
echo "[4/4] done. golden MiniSat traces in $golddir"
echo
