...
/**
 * Runtime benchmark for the Kotlin [MiniSat] port (JVM).
 *
 * Solves each CNF passed on the command line (or, with no args, every
 * every CNF under `shadow/cnf/` found by walking up from the working dir) and prints one line
 * per instance with the verdict and the median solve time over a few timed runs
 * after warm-up (JVM JIT needs warming before numbers mean anything).
 *
 * The companion script `shadow/tools/benchmark.sh` builds the C MiniSat
 * (`clang++ -O2 minisat_trace.cc`), times it on the SAME CNFs, and prints a
 * side-by-side table so we can confirm the port is within a small constant factor
 * of C. No perf claims without these measured numbers.
 *
 * Usage: ./gradlew :minisat:runBenchmark
 */
object Benchmark {
    ...
    fun main(args: Array<String>) {
        ...
        println("# Kotlin MiniSat benchmark (solve-only, median of $RUNS runs, $WARMUP warm-up)")
